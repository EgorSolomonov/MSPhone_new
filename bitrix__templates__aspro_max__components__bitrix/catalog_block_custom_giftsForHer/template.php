...
$this->addExternalJS(SITE_TEMPLATE_PATH . "/components/bitrix/catalog.section/catalog_block_custom_giftsForHer/script_carousel.js");
// Подключаю ФЭНСИ БОКС
?>
<? if ($arResult["ITEMS"]) : ?>
	<?
		...
		<? if (!$bSlide) : ?>
			...
			<div class="top_wrapper items_wrapper <?= $templateName; ?>_template <?= $arParams['IS_COMPACT_SLIDER'] ? 'compact-catalog-slider' : '' ?>">
				...
				<? foreach ($arResult["ITEMS"] as $arItem) {
					if ($arItem["PRODUCT"]["AVAILABLE"] === "Y") { // добавил фильтр для товаров в наличии
				?>
												...
												<div class="counter_wrapp ce_cmp_visible">
													<div id="<?= $arItemIDs["ALL_ITEM_IDS"]['BASKET_ACTIONS']; ?>" class="button_block wide">
														<? if ($bOutOfProduction && $arItem['PROPERTIES']['PRODUCT_ANALOG_FILTER']['VALUE']) : ?>
															<a class='btn btn-transparent-border-color btn-wide has-ripple btn--wrap-text fill-dark-light-block' href='<?= $arItem['PROPERTIES']['PRODUCT_ANALOG_FILTER']['VALUE']; ?>' title='<?= GetMessage('EXPRESSION_OUT_OF_PRODUCTION_TEXT'); ?>'>
